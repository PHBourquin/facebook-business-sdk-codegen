...
const PATH_SKIP_AUTOGEN = [/^(.*)\/[sS]erver[_]?[sS]ide\/(.*)$/, /^(.*)\/\.travis.yml$/, /^(.*)\/\.github\/workflows\/(.*).yml$/, /^(.*)\/\keyring.gpg.enc$/, /^(.*)\/\.travis.settings.xml$/, /^(.*)\/facebook_business\/test\/(.*)$/, /^(.*)\/CHANGELOG.md$/, /^(.*)\/[bB]usiness[dD]ata[aA][pP][iI]\/(.*)$/, /^(.*)\/[sS]ignal\/(.*)$/, /^(.*)\/README.md$/, /^(.*)\/release.sh$/, /^(.*)\/\.flowconfig$/, /^(.*)\/facebookbusiness.gemspec$/, /^(.*)\/Gemfile(.*)$/, /^(.*)\/Rakefile$/, /^(.*)\/facebookbusiness.gemspec$/, /^(.*)\/customtype$/, /^(.*)\/csproj$/];
const Utils = {
